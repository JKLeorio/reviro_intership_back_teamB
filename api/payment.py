...
async def subscription_create(
    ...
    await validate_related_fields(
        {Course: subscription_create.course_id, User: subscription_create.owner_id},
        session=session,
    )
...
@payment_router.get(
    "/{payment_id}", response_model=PaymentResponse, status_code=status.HTTP_200_OK
)
async def payment_detail(
    ...
    payment = await session.get(
        Payment,
        payment_id,
        options=[selectinload(Payment.owner), selectinload(Payment.subscription)],
    )
    if not payment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@payment_router.post(
    "/", response_model=PaymentResponse, status_code=status.HTTP_200_OK
)
async def payment_create(
    ...
    """
    await validate_related_fields(
        {User: payment_create.owner_id, Subscription: payment_create.subscription_id},
        session=session,
    )
